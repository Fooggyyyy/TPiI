...
public class Programm
{
    public static void Main(string[] args)
    {
        ...
        using (IRepository repository = Repository.Create("Celebrities"))
        {
            ...
            app.MapGet("/", () => repository.getAllCelebrities());
            ...
            Validation.SurnameFilter.Repositoryy = Validation.PhotoExistFilter.Repositoryy = repository;
            app.MapPost("/", (Celebrity celebrity) =>
            {
            ...
            }).AddEndpointFilter(async (context, next) =>
            {
                ...
                Console.WriteLine($"{DateTime.UtcNow}: {method} /celebrities/{id} -> {result}");
                return result;
            });


            app.MapPut("/{id:int}", (int id, Celebrity celebrity) =>
            {
            ...
            }).AddEndpointFilter(async (context, next) =>
            {
                ...
                if(celebrity.id < 0) throw new NullFieldAddException("/Celebrities error, Id is wrong");
                if (celebrity.Surname.Length < 2) throw new NullFieldAddException("/Celebrities error, Surname is wrong");
                if (celebrity.Surname.Length > 20) throw new NullFieldAddException("/Celebrities error, Surname is wrong");
